...
paramfilename=$2
if [ $# -eq 4 ]; then
  python -u main_gpu.py --nOctave $nOctave --binsPerOctave $binsPerOctave --timeDepth $timeDepth --nHarmonics $nHarmonics --hopSize $hopSize --voicing $voicing --filterShape1 $filterShape1 --filterShape2 $filterShape2 --filterShape3 $filterShape3 --filterShape4 $filterShape4 --filterShape2D $filterShape2D --filterShape1D $filterShape1D --featureMaps1D $featureMaps1D --featureMaps2D $featureMaps2D --batchSize $batchSize --nEpochs $nEpochs --expName $expname --nUnits $nUnits --expDir $expdir --seqNumber $seqNumber --stateFull $stateFull --model $4 |& tee $expdir/out.log
elif [ $# -eq 5 ]; then
